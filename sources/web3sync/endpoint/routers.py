...
class web3sync_router_builder(router_builder_baseTemplate):
    def __init__(
        self, dex: Protocol, chain: Chain, tags: list | None = None, prefix: str = ""
    ):
    ...
    def _create_routes_hypervisor(
        self, router: APIRouter, dex: Protocol, chain: Chain
    ) -> APIRouter:
    ...
    async def hypervisor_uncollected_fees(
        ...
        return await hypervisors.hypervisor_uncollected_fees(
            chain=self.chain, protocol=self.dex, hypervisor_address=hypervisor_address
        )
    ...
    async def hypervisors_list(self, response: Response):
        ...
        return await hypervisors.hypervisors_list(chain=self.chain, protocol=self.dex)
